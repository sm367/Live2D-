...
class GeminiProvider(BaseLLMProvider):
    ...
    def _history_to_internal_messages(self, history: list[Any], system_instruction: str | None) -> list[dict]:
        ...
        for item in history or []:
            raw_role = getattr(item, "role", "model")
            ...
            # 空パーツのメッセージを履歴に混ぜると次リクエストで不正履歴になりやすい
            if parts_list or content_text:
                out.append(
                    {
                        "role": role,
        ...
        try:
            ...
            while True:
                ...
                if isinstance(item, Exception):
                    raise item
                streamed_text_chunks.append(item)
                yield item

            # ストリーミングは chat 履歴オブジェクトの仕様差分で壊れやすいため、
            # 実際にyieldしたテキストのみを assistant 発話として追記する。
            if streamed_text_chunks:
                messages.append(
                    {
                        "role": "assistant",
                        "content": "".join(streamed_text_chunks),
                        "parts": [{"text": "".join(streamed_text_chunks)}],
                    }
                )
